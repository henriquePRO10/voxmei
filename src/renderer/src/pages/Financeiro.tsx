...
import { snapshotTo } from '../lib/utils'
...
export function Financeiro() {
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; item: Lancamento } | null>(
    null
  )
  const [confirmDelete, setConfirmDelete] = useState<Lancamento | null>(null)
  const [nfConfirm, setNfConfirm] = useState<{
    isOpen: boolean
    message: string
    confirmLabel: string
  }>({
  ...
  const nfConfirmResolverRef = useRef<((value: boolean) => void) | null>(null)
  ...
  const askNfConfirmation = useCallback((message: string, confirmLabel: string) => {
    setNfConfirm({ isOpen: true, message, confirmLabel })
    return new Promise<boolean>((resolve) => {
      nfConfirmResolverRef.current = resolve
    })
  }, [])

  const handleNfConfirmClose = useCallback((confirmed: boolean) => {
    setNfConfirm({ isOpen: false, message: '', confirmLabel: 'Confirmar' })
    if (nfConfirmResolverRef.current) {
      nfConfirmResolverRef.current(confirmed)
      nfConfirmResolverRef.current = null
    }
  }, [])

  const deleteNfFromStorage = useCallback(async (nfUrl?: string) => {
    if (!nfUrl) return
    try {
      const fileRef = ref(storage, nfUrl)
      await deleteObject(fileRef)
    } catch (error) {
      console.warn('Não foi possível excluir anexo do Storage:', error)
    }
  }, [])
  ...
  const fetchLancamentos = useCallback(async () => {
    ...
    try {
      ...
      let data = snapshotTo<Lancamento>(snap)
      data = data.sort(
        (a, b) =>
  ...
  const filteredLancamentos = useMemo(() => {
    return lancamentos.filter((l) => {
      ...
      const tipoMatch = filterTipo ? l.tipo === filterTipo : true
      return mesMatch && anoMatch && clienteMatch && tipoMatch
    })
  }, [lancamentos, filterMesAno, filterCliente, filterTipo])
  ...
  const lancamentosByCliente = useMemo(() => {
    const map = new Map<
      string,
      { nomeFantasia: string; receitas: number; despesas: number; items: Lancamento[] }
    >()
  ...
  }, [filteredLancamentos, clienteNomeById])
  ...
  const handleDeleteLancamento = async () => {
    ...
    try {
      await deleteNfFromStorage(confirmDelete.nfUrl)
  ...
  const onSubmitLancamento = async (data: FormLancamento) => {
    ...
    const hasExistingNf = Boolean(editingLancamento?.nfUrl)
    const isReplacingNf = isEditing && hasExistingNf && data.hasNf && Boolean(data.nfFile?.length)
    const isRemovingNf = isEditing && hasExistingNf && !data.hasNf

    if (isReplacingNf) {
      const confirmed = await askNfConfirmation(
        'Ao salvar, o anexo atual será substituído e o arquivo antigo será apagado. Deseja continuar?',
    ...
    if (isRemovingNf) {
      const confirmed = await askNfConfirmation(
        'Ao salvar sem NF, o anexo atual será apagado permanentemente. Deseja continuar?',
        'Apagar e continuar'
      )
      if (!confirmed) return
    }
    ...
    try {
      if (data.hasNf && data.nfFile && data.nfFile.length > 0) {
        if (editingLancamento?.nfUrl) {
          await deleteNfFromStorage(editingLancamento.nfUrl)
        }
        const file = data.nfFile[0]
        const storageRef = ref(
          storage,
          `notas_fiscais/${currentUser.uid}/${Date.now()}_${file.name}`
        )
    ...
    <>
        ...
        <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              Registre e acompanhe as movimentações financeiras do período
            </p>
          </div>
          <MonthPicker value={filterMesAno} onChange={setFilterMesAno} accent="blue" />
        </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredLancamentos.length === 0 ? (
                      ...
                      filteredLancamentos.map((item) => (
                        <tr
                          key={item.id}
                          className="group hover:bg-slate-50/80 transition-colors select-none"
                          onContextMenu={(e) => {
                            e.preventDefault()
                        ...
                        >
                          ...
                          <td className="px-6 py-3.5">
                            <span
                              className={`flex items-center gap-1.5 text-sm font-medium ${item.tipo === 'Receita' ? 'text-emerald-600' : 'text-rose-600'}`}
                            >
                              {item.tipo === 'Receita' ? (
                          ...
                          <td className="px-6 py-3.5 text-center">
                            {item.hasNf && item.nfUrl ? (
                              <a
                                ...
                                className="inline-flex items-center gap-1 text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg text-xs font-medium"
                              >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              {grupo.items.length} lançamento(s)
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
                          <div className="text-right hidden sm:block">
                            ...
                            <p className="text-sm font-semibold text-emerald-600">
                              R$ {grupo.receitas.toFixed(2)}
                            </p>
                          </div>
                          <div className="text-right hidden sm:block">
                            <p className="text-xs text-slate-400">Despesas</p>
                            <p className="text-sm font-semibold text-rose-600">
                              R$ {grupo.despesas.toFixed(2)}
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="text-xs text-slate-400">Saldo</p>
                            <p
                              className={`text-sm font-bold ${saldoGrupo >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                            >
                              R$ {saldoGrupo.toFixed(2)}
                            </p>
                          </div>
                          {isOpen ? (
                            ...
                            <tbody className="divide-y divide-slate-100">
                              {grupo.items
                                ...
                                .map((item) => (
                                  <tr
                                    key={item.id}
                                    className="group hover:bg-white transition-colors select-none"
                                    onContextMenu={(e) => {
                                      e.preventDefault()
                                      setContextMenu({ x: e.clientX, y: e.clientY, item })
                                    }}
                                  >
                                    ...
                                    <td className="px-6 py-2.5">
                                      <span
                                        className={`flex items-center gap-1 text-xs font-semibold ${item.tipo === 'Receita' ? 'text-emerald-600' : 'text-rose-600'}`}
                                      >
                                        {item.tipo === 'Receita' ? (
                                    ...
                                    <td className="px-6 py-2.5 text-center">
                                      {item.hasNf && item.nfUrl ? (
                                        <a
                                          ...
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="inline-flex items-center gap-1 text-blue-600 bg-blue-50 px-2 py-0.5 rounded text-xs font-medium"
                                        >
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h2 className="text-lg font-bold text-slate-800">
                  {editingLancamento ? 'Editar Lançamento' : 'Registrar Movimentação'}
                </h2>
                {editingLancamento && (
                  <p className="text-xs text-slate-400 mt-0.5">{editingLancamento.descricao}</p>
                )}
              </div>
              <button
                onClick={handleCloseLancamentoModal}
                className="text-slate-400 hover:text-slate-600 p-2 bg-white rounded-full shadow-sm cursor-pointer"
              >
                ✕
              </button>
            </div>
            <form
              onSubmit={lancamentoForm.handleSubmit(onSubmitLancamento)}
              className="p-6 space-y-4"
            >
              <div className="grid grid-cols-2 gap-3">
                <label
                  className={`cursor-pointer border-2 rounded-xl p-3.5 text-center transition-all ${watchTipo === 'Receita' ? 'border-emerald-500 bg-emerald-50 text-emerald-700 font-bold' : 'border-slate-100 text-slate-500'}`}
                >
                  <input
                    type="radio"
                    value="Receita"
                    {...lancamentoForm.register('tipo')}
                    className="hidden"
                  />
                ...
                <label
                  className={`cursor-pointer border-2 rounded-xl p-3.5 text-center transition-all ${watchTipo === 'Despesa' ? 'border-rose-500 bg-rose-50 text-rose-700 font-bold' : 'border-slate-100 text-slate-500'}`}
                >
                  <input
                    type="radio"
                    value="Despesa"
              ...
              <div>
                ...
                <select
                  {...lancamentoForm.register('clienteId', { required: true })}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition cursor-pointer"
                >
              ...
              <div>
                ...
                <select
                  {...lancamentoForm.register('categoria', { required: true })}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition cursor-pointer"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    step="0.01"
                    {...lancamentoForm.register('valor', { required: true })}
                    placeholder="0.00"
                ...
                <div>
                  ...
                  <input
                    type="date"
                    {...lancamentoForm.register('data', { required: true })}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition"
                  />
                </div>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-sm font-medium text-slate-700">
                    Possui Nota Fiscal / Comprovante?
                  </span>
                </label>
                {watchHasNf && (
                  <div className="mt-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <input
                      ...
                      {...lancamentoForm.register('nfFile', {
                        required: watchHasNf && !editingLancamento?.nfUrl
                      })}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={handleCloseLancamentoModal}
                  className="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-100 rounded-xl transition cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow-lg shadow-blue-500/30 transition flex items-center gap-2 disabled:opacity-50 cursor-pointer"
                >
                  {uploadingImage ? (
      ...
      {contextMenu && (
        ...
        >
          ...
          <button
            onClick={() => {
              setConfirmDelete(contextMenu.item)
              setContextMenu(null)
            }}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-slate-500 ml-13">
              &ldquo;<span className="font-medium text-slate-700">{confirmDelete.descricao}</span>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-semibold text-slate-800">Confirmar alteração de anexo</h3>
            </div>
            <p className="text-sm text-slate-500 ml-13">{nfConfirm.message}</p>
            <div className="flex justify-end gap-3 mt-6">
              <button
                onClick={() => handleNfConfirmClose(false)}
                className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition cursor-pointer"
              >
                Cancelar
              </button>
              <button
                onClick={() => handleNfConfirmClose(true)}
                className="px-5 py-2 text-sm font-semibold bg-amber-600 hover:bg-amber-700 text-white rounded-xl shadow-sm shadow-amber-500/30 transition cursor-pointer"
              >
                {nfConfirm.confirmLabel}
              </button>
            </div>
          </div>
        </div>
