...
import { Settings, PlayCircle, Loader2, CheckCircle2, Search, Users, Calculator, AlertCircle } from 'lucide-react';
...
export function ProLabore() {
    ...
    const handleGenerateValues = async () => {
        ...
        try {
            ...
            const selectedAndExisting = Array.from(selectedClientes).filter(id => existingClientIds.has(id));
    ...
    const executeGeneration = async () => {
        ...
        try {
            ...
            setShowToast(true);
            setTimeout(() => { setSuccess(false); setShowToast(false); }, 4000);
        } catch (error) {
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col">
            ...
            <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100 shrink-0">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-0.5">Configure os valores e selecione os clientes para a competência atual</p>
                </div>
                <MonthPicker
                        value={currentMonth}
                ...
                <div className="lg:col-span-4 flex flex-col gap-4 h-full">
                    ...
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 relative overflow-hidden shrink-0">
                        ...
                        <div className="space-y-4 relative z-10">
                            <div>
                                <label className="block text-xs font-medium text-slate-700 mb-1">Salário Base (R$)</label>
                                <input
                                    type="number"
                                    step="0.01"
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <label htmlFor="temInss" className="text-sm font-medium text-slate-700 cursor-pointer">
                            ...
                            {temInss && (
                                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Alíquota INSS (%)</label>
                                    <input
                                        type="number"
                                        placeholder="preencha aqui"
                                        {...register('inssPerc')}
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-medium px-2">
                                    Gerar pró-labore para <strong className="text-white">{selectedClientes.size}</strong> clientes.
                                </p>
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                        ...
                        <p className="text-slate-600 mb-6">
                            <strong className="text-slate-900">{existingCount}</strong> dos clientes selecionados já possuem pró-labore gerado para <strong className="text-slate-900">{currentMonth}</strong>. Deseja gerar novamente e sobrescrever os registros existentes?
                        </p>
            ...
            {showToast && (
                <div className="fixed top-5 right-5 z-100 flex items-center gap-2.5 bg-emerald-600 text-white text-sm font-medium px-4 py-3 rounded-xl shadow-lg shadow-emerald-500/30 pointer-events-none animate-in slide-in-from-top-2 duration-300">
                    ...
                    {toastCount} pró-labore{toastCount !== 1 ? 's' : ''} gerado{toastCount !== 1 ? 's' : ''} com sucesso!
                </div>
            )}
        </div>
    );
