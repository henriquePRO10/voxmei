...
/* ─── helpers ───────────────────────────────────────────────── */
function parseDateAbertura(d: string | undefined): Date | null {
...
export function Relatorios() {
  ...
  useEffect(() => {
    ...
    const fetchClientes = async () => {
      ...
      const data = snapshotTo<Cliente>(snap)
      ...
      try {
        if (tipoRelatorio === 'faturamento') {
          ...
          if (mesesValidos.length > 0) {
            await gerarPdf(cliente, mesesValidos, folderPath)
          }
        } else {
          await gerarRelatorioReceitas(cliente, mesReceitas, folderPath)
        }
      } catch (err: unknown) {
        console.error(
          `Erro ao gerar relatório para ${cliente.nomeFantasia}:`,
  ...
  const gerarPdf = async (
    ...
    try {
      ...
      page.drawText(cnpjSigText, {
        x: sigCX + Math.max(0, (sigW - cnpjSigW) / 2),
        y: clienteLineY - 26,
        size: 7.5,
        font: fontReg,
        color: gray
      ...
      page.drawText(contadorNome, {
        x: sigCX + Math.max(0, (sigW - cNomeW) / 2),
        y: contadorLineY - 14,
        size: 8,
        font: fontBold,
        color: black
      ...
      if (crcText) {
        ...
        page.drawText(crcText, {
          x: sigCX + Math.max(0, (sigW - crcW) / 2),
          y: contadorLineY - 26,
          size: 7.5,
          font: fontReg,
          color: gray
      ...
      const competencia =
        competenciaInicio && competenciaFim
          ? competenciaInicio === competenciaFim
            ? competenciaInicio
            : `${competenciaInicio}_a_${competenciaFim}`
          : 'periodo'
  ...
  const gerarRelatorioReceitas = async (
    ...
    try {
      ...
      snap.docs.forEach((d) => {
        ...
        if (cat === 'Comércio') {
          hasNf ? (comComNF += valor) : (comSemNF += valor)
        } else if (cat === 'Indústria') {
          hasNf ? (indComNF += valor) : (indSemNF += valor)
        } else if (cat === 'Serviços') {
          hasNf ? (serComNF += valor) : (serSemNF += valor)
        }
      })
      ...
      page.drawLine({
        start: { x: mX, y: tableTopY },
        end: { x: mX + cW, y: tableTopY },
        color: black,
        thickness: 1.2
      })
      page.drawLine({
        start: { x: mX, y: tableBottomY },
        end: { x: mX + cW, y: tableBottomY },
        color: black,
        thickness: 1.2
      })
      page.drawLine({
        start: { x: mX, y: tableBottomY },
        end: { x: mX, y: tableTopY },
        color: black,
        thickness: 1.2
      })
      page.drawLine({
        start: { x: mX + cW, y: tableBottomY },
        end: { x: mX + cW, y: tableTopY },
        color: black,
        thickness: 1.2
      })
      ...
      for (const [top, bottom] of [
        ...
        [seg2Top, seg2Bottom],
        [seg3Top, seg3Bottom],
        [grandTop, grandBottom]
      ]) {
        page.drawLine({
          start: { x: valColX, y: bottom },
          end: { x: valColX, y: top },
          color: black,
          thickness: 0.6
        })
      ...
      console.error(
        'Erro ao gerar relatório de receitas brutas:',
        err instanceof Error ? err.message : err
      )
    }
  }
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100 shrink-0">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Configure e gere relatórios em lote para seus clientes
          </p>
        </div>
      </div>
