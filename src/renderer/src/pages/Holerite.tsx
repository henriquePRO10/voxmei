...
export function Holerite() {
  ...
  const fetchProLabores = useCallback(async () => {
    ...
    try {
      ...
      const data = snapshotTo<ProLabore>(querySnapshot)
      ...
      try {
          ...
          if (enderecoFormatado) {
            ...
            if (partes.length >= 2) {
              ...
              if (partesVirgula.length >= 4) {
                ...
                enderecoFormatado =
                  `${partesVirgula[0]}, ${partesVirgula[1]}, ${partesVirgula[2]}, ${cidade}`.trim()
              }
            }
          }
        ...
        try {
          ...
          try {
            ...
            const widgets = (razaoSocialField as any).getWidgets()
        ...
        try {
          form.getTextField('nome_cliente').setText(item.nomeFantasia)
        } catch {
          /* noop */
        }
        try {
          form
            .getTextField('Total_Proventos')
            .setText(item.salarioMinimo.toFixed(2).replace('.', ','))
        } catch {
          /* noop */
        }
        try {
          form.getTextField('proventos').setText(item.salarioMinimo.toFixed(2).replace('.', ','))
        } catch {
          /* noop */
        }
        try {
        ...
        try {
          form.getTextField('dias_referentes').setText('30')
        } catch {
          /* noop */
        }
        try {
          form.getTextField('mes_ano_referente').setText(item.mesAno)
        } catch {
          /* noop */
        }
        ...
        if (item.valorInss > 0) {
          try {
            form.getTextField('cod_inss').setText('3')
          } catch {
            /* noop */
          } // Código genérico para INSS, ajuste se necessário
          try {
            form.getTextField('descricao_inss').setText('INSS')
          } catch {
            /* noop */
          }
          try {
            form.getTextField('referencia_inss').setText(`${item.inssPerc}%`)
          } catch {
            /* noop */
          }
          try {
            form.getTextField('valor_inss').setText(item.valorInss.toFixed(2).replace('.', ','))
          } catch {
            /* noop */
          }
          try {
            form.getTextField('total_desconto').setText(item.valorInss.toFixed(2).replace('.', ','))
          } catch {
            /* noop */
          }
        } else {
          ...
          try {
            form.getTextField('cod_inss').setText('')
          } catch {
            /* noop */
          }
          try {
            form.getTextField('descricao_inss').setText('')
          } catch {
            /* noop */
          }
          try {
            form.getTextField('referencia_inss').setText('')
          } catch {
            /* noop */
          }
          try {
            form.getTextField('valor_inss').setText('')
          } catch {
            /* noop */
          }
          try {
            form.getTextField('total_desconto').setText('')
          } catch {
            /* noop */
          }
        }
        ...
        page.drawText(`Salário Base: R$ ${item.salarioMinimo.toFixed(2)}`, {
          x: 50,
          y: 680,
          size: 12
        })
        page.drawText(`INSS Retido: R$ ${item.valorInss.toFixed(2)}`, { x: 50, y: 660, size: 12 })
        page.drawText(`Líquido a Receber: R$ ${item.valorLiquido.toFixed(2)}`, {
          x: 50,
          y: 640,
          size: 14
        })
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-100 shrink-0">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Visualize e emita os recibos de pró-labore gerados
          </p>
        </div>

        <div className="flex items-center gap-3 w-full sm:w-auto">
          <MonthPicker value={filterMonth} onChange={setFilterMonth} accent="indigo" />
        </div>
      </div>
        ...
        <div className="flex-1 overflow-auto p-1">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-400 space-y-2 p-6 text-center">
              ...
              <p className="text-xs">
                Não há registros gerados para a competência {filterMonth} ou para a sua busca.
              </p>
            </div>
          ) : (
              ...
              <thead className="sticky top-0 bg-white/90 backdrop-blur-sm z-10 shadow-sm">
                <tr className="text-slate-500 font-medium border-b border-slate-100">
                  <th className="p-3 w-10 text-center">
                    ...
                    >
                      {selectedItems.size === filteredProLabores.length &&
                      filteredProLabores.length > 0 ? (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        Base: R$ {item.salarioMinimo.toFixed(2)}
                      </div>
                    </td>
