...
export function Clientes() {
    ...
    const fetchClientes = async () => {
        ...
        try {
            ...
            let data = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Cliente[];
    ...
    const handleCloseModal = () => {
        ...
        reset({
            cnpj: '', razaoSocial: '', nomeFantasia: '', telefone: '', email: '', atividadePrincipal: '',
            optanteSimples: false, enderecoCompleto: '', dataAbertura: '', situacaoCadastral: '', naturezaJuridica: '', status: 'Ativo'
        });
    };
                        ...
                        <tbody className="divide-y divide-slate-100">
                            ...
                            {clientes.map((cliente) => (
                                <tr key={cliente.id} className="hover:bg-slate-50/80 transition-colors group select-none" onContextMenu={e => { e.preventDefault(); setContextMenu({ x: e.clientX, y: e.clientY, cliente }); }}>
                                    ...
                                    <td className="px-6 py-4 font-medium">{cliente.cnpj}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col gap-1 items-start">
                                            <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${cliente.status === 'Ativo' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' :
                                                cliente.status === 'Pendente' ? 'bg-amber-50 text-amber-600 border-amber-200' :
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-medium text-slate-800">{cliente.telefone || 'S/ telefone'}</div>
                                        {cliente.email && <div className="text-xs text-slate-500 mt-0.5">{cliente.email}</div>}
                                    </td>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                ...
                                <div className="col-span-1 md:col-span-2">
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Busca Automática (CNPJ)</label>
                                ...
                                <div className="col-span-1">
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Status Sistema</label>
                                    <select
                                        {...register('status')}
                                ...
                                <div className="col-span-1 md:col-span-1">
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Data Abertura</label>
                                    <input
                                ...
                                <div className="col-span-1 md:col-span-2">
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Natureza Jurídica</label>
                                    <input
                                ...
                                <div className="col-span-1">
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Situação Cadastral</label>
                                    <input
